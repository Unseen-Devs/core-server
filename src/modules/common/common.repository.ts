...
import { createPaginationObject, resolveOptions } from 'src/helpers/resolve-pagination';
...
export class CommonRepository<Model extends ObjectLiteral> extends Repository<Model> {
  async paginate(
    ...
    cb?: (val: FindOneOptions<Model> | FindManyOptions<Model>) => FindOneOptions<Model> | FindManyOptions<Model>,
  ): Promise<Pagination<Model>> {
    const { page, limit, query } = resolveOptions(data);
    const options = cb ? cb({ ...searchOptions, where: query }) : { ...searchOptions, where: query };
